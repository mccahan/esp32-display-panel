...
router.put('/:id', async (req, res) => {
    ...
    const updated = (0, deviceService_1.updateDeviceConfig)(req.params.id, req.body);
    if (!updated) {
...
router.post('/:id/config', async (req, res) => {
    ...
    if (Object.keys(req.body).length > 0) {
        (0, deviceService_1.updateDeviceConfig)(req.params.id, req.body);
    }
...
router.get('/:id/state', async (req, res) => {
    ...
    const state = await (0, deviceService_1.fetchDeviceState)(device);
    if (state) {
        res.json(state);
    }
    else {
...
router.delete('/:id', (req, res) => {
    const success = (0, db_1.deleteDevice)(req.params.id);
    if (success) {
...
router.post('/:id/screenshot/capture', async (req, res) => {
    const device = (0, db_1.getDevice)(req.params.id);
    if (!device) {
        return res.status(404).json({ error: 'Device not found' });
    }
    const success = await (0, deviceService_1.captureDeviceScreenshot)(device);
    if (success) {
...
router.get('/:id/screenshot', async (req, res) => {
    ...
    const screenshot = await (0, deviceService_1.getDeviceScreenshot)(device);
    if (screenshot) {
...
router.post('/:id/brightness', async (req, res) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            (0, deviceService_1.updateDeviceConfig)(device.id, { display: { ...device.config.display, brightness } });
...
router.post('/:id/theme', async (req, res) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            (0, deviceService_1.updateDeviceConfig)(device.id, { display: { ...device.config.display, theme } });
...
router.get('/:deviceId/buttons/:buttonId/binding', (req, res) => {
    const device = (0, db_1.getDevice)(req.params.deviceId);
    if (!device) {
        return res.status(404).json({ error: 'Device not found' });
    }
...
router.post('/:deviceId/buttons/:buttonId/binding', (req, res) => {
    const device = (0, db_1.getDevice)(req.params.deviceId);
    if (!device) {
        return res.status(404).json({ error: 'Device not found' });
    }
...
router.delete('/:deviceId/buttons/:buttonId/binding', (req, res) => {
    const device = (0, db_1.getDevice)(req.params.deviceId);
    if (!device) {
        return res.status(404).json({ error: 'Device not found' });
    }
